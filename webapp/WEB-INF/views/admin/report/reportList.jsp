...
<div class="container-fluid">
	...
	<!-- Page Heading -->
	<h1 class="h3 mb-2 text-gray-800">신고 접수내역</h1>
	...
	<table id="dataTable" class="ui celled table"
		...
		<tfoot>
			<tr>
				<th colspan="7" style="text-align: right; white-space: nowrap;">
					신고 접수 현황 <label style="color: red;">${pageMaker.total}</label> 개
				</th>
			<tr>
		</tfoot>
		...
		<tbody>
			<c:forEach var="report" items="${report}">
				<tr>
					<td><c:out value="${report.r_no}" /></td>
					<td><c:out value="${report.r_writer}" /></td>
					<td><a href="${report.r_no}" class="move"><c:out
								value="${report.r_contents}" /></a></td>
					<td><c:out value="${report.r_reason}" /></td>
			...
			function() {
				...
				$(".move").on(
						...
						function(e) {
							e.preventDefault();
							actionForm
									.append("<input type='hidden' name='r_no' "
											+ "value='" + $(this).attr("href")
											+ "'>");
							actionForm.attr("action",
									"/admin/report/reportView");
							actionForm.submit();
						});
